...
git_make_function gbda 'git branch --no-color --merged | command grep -vE \'^(+|*|\s*((git_main_branch)|development|develop|devel|dev)\s*$)\' | command xargs -n 1 git branch -d'
git_make_function gbD 'git branch -D'
...
git_make_function gcd 'git checkout develop'
...
git_make_function grbd 'git rebase develop'
